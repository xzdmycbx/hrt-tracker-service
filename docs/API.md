...
- **设备限制**：每个账户最多同时在 2 个设备上登录，第 3 个设备登录时，最早的设备会自动失效
...
- 刷新令牌的过期时间由环境变量 `REFRESH_TOKEN_EXPIRE_HOURS` 配置，默认为 168 小时（7天）。
...
**说明：**
- **设备限制**：每个账户最多同时在 2 个设备登录。当第 3 个设备登录时，最早登录的设备的 Refresh Token 会自动失效。
...
2. **刷新令牌（Refresh Token）**：默认7天过期。用于获取新的访问令牌。
